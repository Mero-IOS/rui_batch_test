...
public class RuiMandatiReader {
    ...
    @Bean
    @StepScope
    public FlatFileItemReader<RuiMandati> readerRuiMandati(
        ...
        return new FlatFileItemReaderBuilder<RuiMandati>()
                .name("readerRuiMandati")
                .resource(new FileSystemResource(createFileName(outputPath, FILE_NAME_RUI_MANDATI, date)))
                .recordSeparatorPolicy(new BlankLineRecordSeparatorPolicy())
                .lineMapper(new CustomLineMapper<>(RuiMandati.class).mapper(NAMES))
                .linesToSkip(1)
                .build();
    }
}
