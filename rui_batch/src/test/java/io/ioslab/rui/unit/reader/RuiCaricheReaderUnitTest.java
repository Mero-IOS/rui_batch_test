...
import io.ioslab.rui.batch.reader.RuiCaricheReader;
...
class RuiCaricheReaderUnitTest {

    private FlatFileItemReader<RuiCariche> flatFileItemReader;
    ...
    void setFlatFileItemReader() throws IOException {
        flatFileItemReader = new RuiCaricheReader().readerRuiCariche(
            mockCsvWithValidAndInvalidRecords.getFile().getPath(), CSV_FILE_NAME_DATE);
        chunkContextMock = mockChunkContext();
    }

    @Test
    void readerRuiCariche_invalidCsvDate_throwsExceptionInOpening() throws IOException {
        flatFileItemReader = new RuiCaricheReader().readerRuiCariche(
    ...
    @Test
    void readerRuiCariche_fromValidCsv_doesRead() throws Exception {
        flatFileItemReader.afterPropertiesSet();
        flatFileItemReader.open(getExecutionContext(chunkContextMock));
        assertThat(flatFileItemReader.read()).isInstanceOf(RuiCariche.class);
    }

    @Test
    void readerRuiCariche_fromValidCsv_throwsExceptionOnLastNonValidIgnoringDoubleReturn()
        throws Exception {
        flatFileItemReader.afterPropertiesSet();
        flatFileItemReader.open(getExecutionContext(chunkContextMock));
        int lastIndex = 0;
    ...
    @Test
    void readerRuiCariche_fromElevenInvalidRecordCsv_throwsElevenExceptions() throws Exception {
        flatFileItemReader = new RuiCaricheReader().readerRuiCariche(
