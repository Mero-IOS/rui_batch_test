...
class RuiIntermediariWriterUnitTest {

    private JdbcBatchItemWriter<RuiIntermediari> writer;
    @Autowired
    private DataSource dataSource;

    private List<RuiIntermediari> intermediariList;

    @BeforeEach
    void setWriter() {
        dataSource = new CustomDataSource().getDataSourceTest();
        writer = new RuiIntermediariWriter().writerRuiIntermediari(dataSource);
    ...
    private void setList() {
        intermediariList = new ArrayList<>();
        for (int i = 0; i < VALID_RECORD_PER_CSV; i++) {
            RuiIntermediari toAdd = new RuiIntermediari();
            toAdd.setId(i);
            toAdd.setDataElaborazione(CSV_DATE_AS_SQL_DATE);
            intermediariList.add(toAdd);
        }
    }
